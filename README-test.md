...
- The PD callback URL (used by Mirth's HTTP Sender) is read from `PD_CALLBACK_URL` (fallback to `http://100.27.251.103:8000/api/pd/callback`).
...
2. Set `PD_ENDPOINT_URL` and `PD_CALLBACK_URL` in `.env` if you need to override the default test endpoints.
